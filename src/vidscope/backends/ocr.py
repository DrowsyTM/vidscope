...
class OcrBackendFailure(RuntimeError):
    def __init__(self, code: str, message: str) -> None:
        self.error = AnalysisError(
            code=ErrorCode(code), stage="ocr", message=message, retryable=False
        )
...
DEFAULT_OCR_TIMEOUT: float = 60.0
...
class TesseractBackend:
    ...
    def _binary(self) -> str:
        configured = (
            self.binary
            or _field(self.settings, "tesseract_bin", None)
            or _field(get_settings(), "tesseract_bin", None)
        )
        value = str(configured) if configured else shutil.which("tesseract")
        if not value:
            raise OcrBackendFailure(
                "OCR_UNAVAILABLE", "tesseract binary is unavailable"
            )
        return value

    def recognize(
        self, frame: Any, language: str = "eng", request: Any = None, **_: Any
    ) -> OcrArtifact:
        frame_path = Path(
            frame if isinstance(frame, (str, Path)) else _field(frame, "path", frame)
        )
        ...
        tessdata = _field(self.settings, "tessdata_prefix", None) or _field(
            get_settings(), "tessdata_prefix", None
        )
        if tessdata:
        ...
        try:
            ...
            raise OcrBackendFailure("TIMEOUT", "tesseract timed out") from exc
        except OSError as exc:
            raise OcrBackendFailure(
                "OCR_UNAVAILABLE", "tesseract could not be started"
            ) from exc
        returncode = int(
            getattr(
                result,
                "returncode",
                result.get("returncode", 0) if isinstance(result, Mapping) else 0,
            )
        )
        stdout = getattr(
            result,
            "stdout",
            result.get("stdout", "") if isinstance(result, Mapping) else "",
        )
        stderr = getattr(
            result,
            "stderr",
            result.get("stderr", "") if isinstance(result, Mapping) else "",
        )
        if isinstance(stdout, bytes):
            stdout = stdout.decode("utf-8", errors="replace")
        if returncode:
            raise OcrBackendFailure(
                "OCR_UNAVAILABLE", f"tesseract failed: {str(stderr)[:512]}"
            )
        try:
            ...
            raise OcrBackendFailure(
                "INTERNAL_STAGE_FAILED", "tesseract returned malformed TSV"
            ) from exc
        if not rows:
